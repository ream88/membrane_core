defmodule Membrane.Core.TimerController do
  ...
  def start_timer(id, interval, clock, state) do
    synchronization = StateDispatcher.get_any(state, :synchronization)
    ...
    if synchronization.timers |> Map.has_key?(id) do
      ...
      new_sync = Bunch.Access.put_in(synchronization, [:timers, id], timer)

      state
      |> StateDispatcher.update_any(synchronization: new_sync)
      ~> {:ok, &1}
    end
  end
  ...
  def stop_timer(id, state) do
    old_sync = StateDispatcher.get_any(state, :synchronization)
    {timer, new_sync} = pop_in(old_sync, [:timers, id])
  ...
  def handle_tick(timer_id, state) do
    ...
    withl present?: true <- Map.has_key?(synchronization.timers, timer_id),
      ...
      new_sync = Bunch.Access.update_in(synchronization, [:timers, timer_id], &Timer.tick/1)
  ...
  def handle_clock_update(clock, ratio, state) do
    new_sync =
      Map.update!(
        StateDispatcher.get_any(state, :synchronization),
        :timers,
